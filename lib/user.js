...
let welcomeText = require('./welcome');
let g_combat;
let g_roomManager;
...
let User = function (socket) {
    ...
    this.obj.displayName = socket.request.session.passport.user.displayName.split(" ")[0];
    this.socket = socket;
    ...
    socket.SendMsg(welcomeText + '<br/>' + this.obj.displayName + "님 안녕하세요.<br/>");
};
